...
export class RegistroComponent {
  ...
  checkPasswords: ValidatorFn = (
    ...
    const password_repeat = control.get('contraseña2');
  ...
  checkData() {
    ...
    if (this.citaForm.valid) {
      ...
      this.loadingService.hide();
      if (
        JSON.stringify(this.citaForm.errors) == '{"passwordCoincide":false}'
      ) {
        Swal.fire('Las constraseñas no coinciden', '', 'error').then(
          (result) => {
            button.innerHTML = `Ok`;
          }
        );
      } else {
        Swal.fire('¡Porfavor llene todos los campos!', '', 'error').then(
          (result) => {
            button.innerHTML = `Ok`;
          }
        );
  ...
  registrarPaciente(): void {
    ...
    if (
      this.validate_password(this.contrasenaPac) &&
      this.validate_email(this.correoPac) &&
      this.validate_field(this.nombrePac)
    ) {
      this.myAuth
        .register(
          this.correoPac,
        ...
        .subscribe(
          () => {
            Swal.fire({
              title: 'Éxito!',
              text: 'Se ha registrado tu usuario correctamente, ya puedes iniciar sesión. ¡VitalCare te da la bienvenida!.',
              icon: 'success',
